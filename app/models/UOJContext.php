...
class UOJContext {
	...
	public static function pageConfig() {
		if (!isset(self::$data['type'])) {
			...
			return array(
				'PageNav' => 'blog-nav',
				'PageMainTitle' => UOJContext::$data['user']['username'] . UOJLocale::get('his blog'),
				'PageMainTitleOnSmall' => UOJLocale::get('blogs'),
			);
		}
	}
			...
			case 'blog':
				switch ($name) {
					...
					case 'hasBlogPermission':
						return isSuperUser(Auth::user()) || Auth::id() === self::$data['user']['username'];
					case 'isHis':
						if (!isset($args[0])) {
							return false;
						}
