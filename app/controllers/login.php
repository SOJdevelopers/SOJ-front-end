...
function submitLoginPost() {
	...
	$.post('/login', {
		...
		if (msg == 'ok') {
			...
			if (prevUrl == '' || /.*\/login.*/.test(prevUrl) || /.*\/register.*/.test(prevUrl) || /.*\/reset-password.*/.test(prevUrl) || /.*\/logout.*/.test(prevUrl) || prevUrl.indexOf(uojHome) == -1) {
				prevUrl = '/';
			};
