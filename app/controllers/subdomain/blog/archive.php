<?php
	...
	requireLib('shjs');
?>
<?php echoUOJPageHeader(UOJLocale::get('archive')) ?>

<div class="row">
	<div class="col-md-3">
		<?php if (UOJContext::hasBlogPermission()): ?>
		<div class="btn-group btn-group-justified">
			<a href="<?=HTML::blog_url(UOJContext::userid(), '/blog/new/write')?>" class="btn btn-primary"><span class="glyphicon glyphicon-edit"></span> <?= UOJLocale::get('write new blog') ?></a>
			<a href="<?=HTML::blog_url(UOJContext::userid(), '/slide/new/write')?>" class="btn btn-primary"><span class="glyphicon glyphicon-edit"></span> <?= UOJLocale::get('write new slide') ?></a>
		</div>
		<?php endif ?>
		<div class="panel panel-info top-buffer-sm">
			<div class="panel-heading"><?= UOJLocale::get('tags') ?></div>
			...
			<?php endforeach ?>
			<?php else: ?>
				<div class="text-muted"><?= UOJLocale::get('none') ?></div>
			<?php endif ?>
			</div>
		</div>
	</div>
