<?php
	...
	class SyncProblemDataHandler {
		...
		private function compile_at_prepare($name, $config = array()) {
			...
			if ($name === 'std') {
				$extra_arg = '-DONLINE_JUDGE_STANDARD_PROGRAM';
			} else {
				$extra_arg = '';
			}

			if (isset($config['need_include_header']) && $config['need_include_header']) {
				exec("$cmd_prefix --add-readable-raw=$include_path/ /usr/bin/g++ -o $name {$config['src']} -I$include_path -lm -O2 -DONLINE_JUDGE $extra_arg");
			} else {
				exec("$cmd_prefix /usr/bin/g++ -o $name {$config['src']} -lm -O2 -DONLINE_JUDGE $extra_arg");
			}
